...
class Work(models.Model):
    ...
    hz = fields.Selection(selection='_get_works', string='job_id.')

    @api.model
    def _get_works(self):
        result = []
        count = 0
        all_works = self.env['contract.work'].search([])
        for work in all_works:
            if work.get_group:
                print(work.parent_eil_nr)
                id_guide = int(f"{work.get_group}".split(' ')[1])
                id_work = self.env['contract.guide'].browse(id_guide)
                all_numbers = self.env['contract.guide'].search([])
                for number in all_numbers:
                    if int(f"{number.eil_nr}".split('.')[0]) == int(f"{id_work.eil_nr}".split('.')[0]):
                        if number.operations:
                            count += 1
                            result.append((f'operation {number.id}', f'{number.eil_nr} / {number.diameter} {number.operations}'))
        return result
